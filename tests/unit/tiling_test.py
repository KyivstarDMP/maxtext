...
def compute_loss_linen(intermediate_outputs, logits, data, config, model, params, is_train):
  ...
  if config.num_vocab_tiling > 1:
    ...
    total_loss, _, _ = vocab_tiling_linen_loss(hidden_states, data, config, model, params, is_train)
  else:
...
class LossAndGradientCorrectnessTest(unittest.TestCase):
  ...
  def test_vocab_tiling_nnx_loss(self):
    ...
    xent_sum_tiled, _, _ = vocab_tiling_nnx_loss(model, hidden_states, data, cfg, is_train=True)
